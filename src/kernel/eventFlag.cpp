...
int KYTY_SYSV_ABI KernelCreateEventFlag(KernelEventFlag* e,const char* n,uint32_t a,uint64_t b,const void* p){PRINT_NAME();if(!e||!n||p||(a&~0x33u))return KERNEL_ERROR_EINVAL;bool s=(a&0xf0u)!=0x20u,f=(a&0xfu)!=2;if((a&0xfu)>2||(a&0xf0u)>0x20u)return KERNEL_ERROR_EINVAL;*e=g_flags.Insert(std::make_shared<KernelEventFlagPrivate>(n,s,f,b));return OK;}
int KYTY_SYSV_ABI KernelDeleteEventFlag(KernelEventFlag e){PRINT_NAME();auto p=g_flags.Remove(e);if(!p)return KERNEL_ERROR_ESRCH;p->Close();return OK;}
int KYTY_SYSV_ABI KernelWaitEventFlag(KernelEventFlag e,uint64_t b,uint32_t v,uint64_t* o,KernelUseconds* t){PRINT_NAME();if(!b)return KERNEL_ERROR_EINVAL;auto p=g_flags.Acquire(e);if(!p)return KERNEL_ERROR_ESRCH;Decoded d;if(!Decode(v,&d))return KERNEL_ERROR_EINVAL;auto r=p->Wait(b,d.wait,d.clear,o,t);return r==KernelEventFlagPrivate::Result::Ok?OK:r==KernelEventFlagPrivate::Result::AlreadyWaiting?KERNEL_ERROR_EPERM:r==KernelEventFlagPrivate::Result::TimedOut?KERNEL_ERROR_ETIMEDOUT:r==KernelEventFlagPrivate::Result::Canceled?KERNEL_ERROR_ECANCELED:KERNEL_ERROR_EACCES;}
int KYTY_SYSV_ABI KernelPollEventFlag(KernelEventFlag e,uint64_t b,uint32_t v,uint64_t* o){PRINT_NAME();if(!b)return KERNEL_ERROR_EINVAL;auto p=g_flags.Acquire(e);if(!p)return KERNEL_ERROR_ESRCH;Decoded d;if(!Decode(v,&d))return KERNEL_ERROR_EINVAL;auto r=p->Poll(b,d.wait,d.clear,o);return r==KernelEventFlagPrivate::Result::Ok?OK:r==KernelEventFlagPrivate::Result::AlreadyWaiting?KERNEL_ERROR_EPERM:KERNEL_ERROR_EBUSY;}
int KYTY_SYSV_ABI KernelSetEventFlag(KernelEventFlag e,uint64_t b){PRINT_NAME();auto p=g_flags.Acquire(e);if(!p)return KERNEL_ERROR_ESRCH;p->Set(b);return OK;}
int KYTY_SYSV_ABI KernelClearEventFlag(KernelEventFlag e,uint64_t b){PRINT_NAME();auto p=g_flags.Acquire(e);if(!p)return KERNEL_ERROR_ESRCH;p->Clear(b);return OK;}
int KYTY_SYSV_ABI KernelCancelEventFlag(KernelEventFlag e,uint64_t b,int* n){PRINT_NAME();auto p=g_flags.Acquire(e);if(!p)return KERNEL_ERROR_ESRCH;p->Cancel(b,n);return OK;}
}
