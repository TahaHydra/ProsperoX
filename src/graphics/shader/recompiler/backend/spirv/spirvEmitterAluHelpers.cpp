...
uint32_t EmitF32ToF16RneBits(EmitterState& state, uint32_t f32) {
	...
	const auto normal_tie = EmitCompareU32Constant(state, OpIEqual, normal_remainder, 0x1000u);
	const auto normal_odd =
	    EmitCompareU32Constant(state, OpINotEqual, EmitAndConstant(state, normal_mant, 1u), 0u);
	...
	state.builder.AddFunction({OpBitwiseOr, TypeU32(state), normal, sign, rounded_normal_payload});
	...
	state.builder.AddFunction({OpISub, TypeU32(state), sub_mask, sub_range, ConstantU32(state, 1)});
	...
	const auto sub_odd =
	    EmitCompareU32Constant(state, OpINotEqual, EmitAndConstant(state, sub_mant, 1u), 0u);
	...
	const auto subnormal        = state.builder.AllocateId();
	state.builder.AddFunction({OpIAdd, TypeU32(state), rounded_sub_mant, sub_mant, sub_increment});
