...
void GpuResourceManager::PrepareBda(bool writes) {
	std::shared_lock lock(m_mapped_ranges_mutex);
	m_mapped_ranges.ForEach([this](uint64_t start, uint64_t end) {
		m_buffer_cache.SynchronizeBuffersInRange(start, end - start);
	});
	m_fault_process_pending = true;
	m_fault_write_pending   = m_fault_write_pending || writes;
}

void GpuResourceManager::RunGarbageCollector() {
	if (m_fault_process_pending) {
		m_fault_process_pending = false;
		const auto writes       = m_fault_write_pending;
		m_fault_write_pending   = false;
		m_buffer_cache.ProcessFaultBuffer(writes);
	}
